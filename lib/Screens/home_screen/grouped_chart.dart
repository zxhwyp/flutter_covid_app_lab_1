...
class GroupedBarChart extends StatelessWidget {
  ...
  factory GroupedBarChart.withSampleData(Covid19WeekModel covid19weekModel) {
    ...
    if (covid19weekModel != null) {
      covid19weekModel.data.forEach((key, value) {
        deathData.add(OrdinalSales(
            DateUtil.getWeekday(DateUtil.getDateTime(key)).substring(0, 3),
            value["deaths"]));
        confirmedData.add(OrdinalSales(
            DateUtil.getWeekday(DateUtil.getDateTime(key)).substring(0, 3),
            value["total_cases"]));
        recoveredData.add(OrdinalSales(
            DateUtil.getWeekday(DateUtil.getDateTime(key)).substring(0, 3),
            value["recovered"]));
      });
    }
    ...
    return new GroupedBarChart(
      _createSampleData(deathData.reversed.toList(),
          confirmedData.reversed.toList(), recoveredData.reversed.toList()),
      animate: false,
    );
  }
  ...
  static List<charts.Series<OrdinalSales, String>> _createSampleData(
      List<OrdinalSales> deathData,
      List<OrdinalSales> confirmedData,
      List<OrdinalSales> recoveredData) {
    return [
